...
*Note*: All references of _source cluster_, _target cluster_ implicitly mean *AMQ Streams v1.4*, *AMQ Streams v1.7* respectively; unless when otherwise explicitly pointed out.
...
. Make sure _source_ and target clusters are monitored and all key performance indicators(KPI) needed to track mirroring progress are displayed on easy to access dashboards,
...
. Validate _target cluster(AMQ Streams v1.7)_ data retention(TTL) configurations,
...
. Validate all `KafkaTopic` resources have been created on _target cluster(AMQ Streams v1.7)_ with parallel configs as those on _source cluster(AMQ Streams v1.4)_ .
...
. Deploy all applications on the _target cluster(AMQ Streams v1.7)_ and validate they spin up and down with no prohibitive errors,
...
. Deploy MirrorMaker2 on _target cluster(AMQ Streams v1.7)_  and begin the replication,
* depending on data size, data ingestion rate, replication speed, it might take from minutes, days, to weeks before MirrorMaker2 is caught up with the _source cluster(AMQ Streams v1.4)_ .
** use the monitoring dashboards(source and target), kcat, and *kafka cli programs* for validation.

## Cut Over Plan

The plan describes steps involved in switching from the _source cluster(AMQ Streams v1.4)_ over to the _target cluster(AMQ Streams V1.7)_.
...
. Stop MM2 instance
* this action is performed after confirming _target cluster(AMQ Streams v1.7)_ has caught up with _source cluster(AMQ Streams v1.4)_ .
...
. Spin down producers and wait for consumers to drain their respective topics,
...
* this ensures all messages on _source cluster(AMQ Streams v1.4)_  have been processed.

. Spin down *Dam* applications if any
...
In this guide, we have explained the strategies and plans involved in the migration of an _AMQ Streams_ cluster in an _Active/Passive_ setting between two OpenShift installations.
