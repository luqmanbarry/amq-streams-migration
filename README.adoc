...
* link:: https://kafka.apache.org/documentation/#consumerconfigs_auto.offset.reset[Red the Kafka docs for more]

### Pods killed due to Out Of Memory in new cluster

Pods were being terminated in the target cluster, at first we thought it was the JVM eating up all the pod memory. We implemented JVM boundaries whereby we _-Xms_ and _Xmx_ values as argument of the run command of the container image; `java -jar` in this case. Despite this change we still got OOMKilled errors, pod were being killed.
